...
class HistoryModelTest(TestCase):
    """Tests for history models (SearchQuery and ViewHistory)."""  # Тесты моделей истории (поисковые запросы и просмотры)
    ...
    def test_search_query_creation(self):
        """Test creating a search query."""  # Тест создания поискового запроса
    ...
    def test_view_history_creation(self):
        """Test creating a view history record."""  # Тест создания записи истории просмотра
...
class PopularSearchViewTest(APITestCase):
    """Tests for popular search queries endpoint."""  # Тесты популярных поисковых запросов
    ...
    def test_popular_search_returns_top_10(self):
        """Test that the endpoint returns top 10 queries from the last 30 days."""  # Тест возврата топ-10 запросов за последние 30 дней
        # Create 15 distinct queries
        for i in range(15):
            SearchQuery.objects.create(user=self.user, query=f"query{i}")

        # Duplicate the first 5 to make them more frequent
        for i in range(5):
            SearchQuery.objects.create(user=self.user, query=f"query{i}")
    ...
    def test_popular_search_filters_short_queries(self):
        """Test that queries shorter than 3 characters are excluded."""  # Тест фильтрации запросов короче 3 символов
    ...
    def test_popular_search_excludes_old_queries(self):
        """Test that queries older than 30 days are excluded."""  # Тест исключения запросов старше 30 дней
    ...
    def test_popular_search_excludes_deleted(self):
        """Test that soft-deleted queries are excluded."""  # Тест исключения мягко удалённых запросов
